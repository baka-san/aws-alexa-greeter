...
class GreeterTestCase(unittest.TestCase):
  ...
  def test_04_next_quote_intent(self):
    json_data = self.json_data.copy()
    json_data['request']['intent']['name'] = 'NextQuoteIntent'
    json_data['request']['intent']['slots'] = {}
    json_data['session']['attributes'] = GreeterTestCase.prev_response['sessionAttributes']

    response = self.get_response(json_data)
    self.check_valid_response(response,end_session=False)
    # self.assertRegexpMatches(response['response']['reprompt']['outputSpeech']['ssml'],r'^<speak> You can say.*</speak>', 'reprompt text valid')

    json_data['session']['attributes'] = {}
    response = self.get_response(json_data)
